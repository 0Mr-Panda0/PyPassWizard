...
class TestPasswordGenerator:
    ...
    def assert_password_properties(self, password: str, length: int, has_letters: bool, has_digits: bool, has_special: bool) -> None:
        """
        ...
        assert any(char.isalpha() for char in password) == has_letters, "Password letter presence mismatch."
        assert any(char.isdigit() for char in password) == has_digits, "Password digit presence mismatch."
        assert any(char in punctuation for char in password) == has_special, "Password special character presence mismatch."

    def test_is_valid_with_valid_length(self):
        """
        Test if the password length is valid.
        """
        assert self.generator.is_valid(int(os.getenv('MIN_LENGTH'))) is True
        assert self.generator.is_valid(int(os.getenv('MAX_LENGTH'))) is True

    def test_is_valid_with_invalid_length(self):
        """
        Test if the password length is invalid.
        """
        with pytest.raises(ValueError, match="Password length should be at least 8 characters."):
            self.generator.is_valid(int(os.getenv('MIN_LENGTH')) - 1)
        with pytest.raises(ValueError, match="Password length should not exceed 128 characters."):
            self.generator.is_valid(int(os.getenv('MAX_LENGTH')) + 1)
    ...
    def test_generate_password(self, length, include_letters, include_digits, include_special, has_letters, has_digits, has_special):
        """
        ...
        self.assert_password_properties(password, length, has_letters, has_digits, has_special)
    ...
    def test_generate_password_with_no_character_types(self):
        ...
        with pytest.raises(ValueError, match="At least one character type must be included."):
            self.generator.generate_password(10, include_letters=False, include_special=False, include_digits=False)